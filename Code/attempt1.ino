...
#define SOL_VOLTS_SCALE 0.02707741477
#define BAT_VOLTS_SCALE 0.02707741477
#define SOL_AMPS_SCALE  0.028722426
...
int read_adc(int channel)
{
  ...
  for (i=0; i<AVG_NUM; i++) { 
    temp = analogRead(channel); sum += temp; delayMicroseconds(50); 
    } 
    return(sum / AVG_NUM); 
    } 
void read_data(void)
{ 
  sol_amps = (read_adc(SOL_AMPS_CHAN) * SOL_AMPS_SCALE )-14.706; 
  if (sol_amps<0){
    sol_amps= -1*sol_amps;
  }
